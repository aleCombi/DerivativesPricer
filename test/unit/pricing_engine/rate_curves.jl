@testitem "Discount Factor Calculation on a spine date" begin
    ...
    date = Date(2023, 1, 1)
    curve = RateCurve(Date(2023, 1, 1), spine_rates; spine_dates=spine_dates)
...
@testitem "Discount Factor Calculation on a non-spine date inputting day-counts" begin
    ...
    spine_rates = [1.5, 2.0, 2.5]
    date = Date(2023, 1, 1)
    curve = RateCurve(Date(2023, 1, 1), spine_rates; spine_day_counts=spine_day_counts)
...
@testitem "Discount Factor Calculation on a spine date, exponential rates" begin
    ...
    spine_rates = [1.5, 2.0, 2.5]
    date = Date(2023, 1, 1)
    curve = RateCurve(Date(2023, 1, 1), spine_rates; spine_dates=spine_dates, rate_type=Exponential())
