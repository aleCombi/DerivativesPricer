...
@testitem "Quantlib: 30360, exp rate, following, 1 year, 3 bd pay delay, target calendar" setup=[QuantlibSetup] begin
    ## Getting Hedgehog Results
    # schedule configuration
    start_date = Date(2019, 6, 27)
    end_date = Date(2029, 6, 27)
    schedule_config = ScheduleConfig(Year(1); business_days_convention=FollowingBusinessDay(), calendar=BusinessDays.TARGET())
    instrument_schedule = InstrumentSchedule(start_date, end_date, schedule_config)
    ...
    # Define schedule with NullCalendar (treat all days as business)
    schedule = ql.Schedule(ql_start_date, ql_end_date, ql.Period(ql.Annual),
                            ql.TARGET(), ql.Following, ql.Following,
                           ql.DateGeneration.Forward, false)

    ql_fixed_rate_leg = ql.FixedRateLeg(schedule, ql.Thirty360(ql.Thirty360.European), [principal], [rate])
    ql_fixed_flows = [cash_flow.amount() for cash_flow in ql_fixed_rate_leg]
