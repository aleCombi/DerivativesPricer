...
function calculate_forward_rate(rate_curve::RateCurve, dates::Vector{D}, rate_config::SimpleRateConfig) where {D<:TimeType}
...
function calculate_forward_rate(rate_curve::RateCurve, dates::Vector{D}, day_count_convention:C, margin_config::M) where {D<:TimeType, M<:MarginConfig, C<:DayCount}
    day_counts = day_count_fraction(dates, day_count_convention)
    discount_factors = discount_factor(rate_curve, dates)
    forward_rate_map = x -> (discount_factors[x+1] / discount_factors[x] - 1) / day_counts[x]
