...
function calculate_expected_flows(stream::FloatingRateStream, forward_rates)
    return calculate_interest(stream.config.principal, forward_rates, stream.accrual_day_counts, stream.config.rate.rate_config.rate_type)
end
...
function calculate_forward_rate(stream::FloatingRateStream, rate_curve::RateCurve)
    return calculate_forward_rate(rate_curve, stream.accrual_dates, stream.fixing_dates, stream.fixing_end_dates, stream.config.rate.rate_config)
end