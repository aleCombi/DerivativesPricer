struct CompoundedRateStreamSchedules{D, T}
    pay_dates::Vector{D}
    compounding_schedules::Vector{SimpleRateStreamSchedules}
...
function CompoundedRateStreamSchedules(stream_config::FlowStreamConfig{P,CompoundInstrumentRate,S}) where {P,S}
    ...
    compounded_instrument_schedules = [InstrumentSchedule(accrual_dates[i], accrual_dates[i+1], stream_config.rate.compound_schedule, stream_config.schedule.pay_shift) for i in 1:length(accrual_dates)-1]
